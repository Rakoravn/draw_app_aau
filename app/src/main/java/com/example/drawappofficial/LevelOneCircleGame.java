...
import static com.example.drawappofficial.models.TensorFlowClassifier.create;

public class LevelOneCircleGame extends Activity implements View.OnClickListener, View.OnTouchListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_level_one_circle_game);

        //get drawing view from XML (where the finger writes the number)
        drawView = (DrawView) findViewById(R.id.circleView);
        ...
        clearBtn = (Button) findViewById(R.id.clearCircleBtn);
        clearBtn.setOnClickListener(this);

        //class button
        //when tapped, this performs classification on the drawn image
        classBtn = (Button) findViewById(R.id.idCircleBtn);
        classBtn.setOnClickListener(this);

        contBtn = (Button) findViewById(R.id.circleContBtn);
        contBtn.setOnClickListener(this);

        contBtn.setEnabled(false);

        resText = (TextView) findViewById(R.id.compCircleTxt);
    ...
    private void loadModel() {
        try {
            mClassifiers = TensorFlowClassifier.create(getAssets(), "Chuni","opt_chuniModel.pb", "labels.txt", PIXEL_WIDTH,
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.idCircleBtn){
            ...
            if (res.getLabel().equals("cirkel")) {
                completed += 1;
                resText.setText(completed + "/3 CIRKLER TEGNET");
        ...
        } else if (v.getId() == R.id.clearCircleBtn){

            drawModel.clear();
            drawView.reset();
            drawView.invalidate();

        } else if (v.getId() == R.id.circleContBtn){
