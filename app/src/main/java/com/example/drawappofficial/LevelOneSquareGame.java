...
public class LevelOneSquareGame extends Activity implements View.OnClickListener, View.OnTouchListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_level_one_square_game);

        //get drawing view from XML (where the finger writes the number)
        drawView = (DrawView) findViewById(R.id.squareView);
        ...
        clearBtn = (Button) findViewById(R.id.clearSquareBtn);
        clearBtn.setOnClickListener(this);

        //class button
        //when tapped, this performs classification on the drawn image
        classBtn = (Button) findViewById(R.id.idSquareBtn);
        classBtn.setOnClickListener(this);

        contBtn = (Button) findViewById(R.id.squareContBtn);
        contBtn.setOnClickListener(this);

        contBtn.setEnabled(false);

        resText = (TextView) findViewById(R.id.compSquareTxt);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.idSquareBtn){
            ...
            if (res.getLabel().equals("firkant")) {
                completed += 1;
                resText.setText(completed + "/3 FIRKANTER TEGNET");
        ...
        } else if (v.getId() == R.id.clearSquareBtn){

            drawModel.clear();
            drawView.reset();
            drawView.invalidate();

        } else if (v.getId() == R.id.squareContBtn){
