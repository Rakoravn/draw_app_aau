...
public class LevelOneTriangleGame extends Activity implements View.OnClickListener, View.OnTouchListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_level_one_triangle_game);

        //get drawing view from XML (where the finger writes the number)
        drawView = (DrawView) findViewById(R.id.triangleView);
        ...
        clearBtn = (Button) findViewById(R.id.clearTriangleBtn);
        clearBtn.setOnClickListener(this);

        //class button
        //when tapped, this performs classification on the drawn image
        classBtn = (Button) findViewById(R.id.idTriangleBtn);
        classBtn.setOnClickListener(this);

        contBtn = (Button) findViewById(R.id.triangleContBtn);
        contBtn.setOnClickListener(this);

        contBtn.setEnabled(false);

        resText = (TextView) findViewById(R.id.compTriangleTxt);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.idTriangleBtn){
            ...
            if (res.getLabel().equals("trekant")) {
                completed += 1;
                resText.setText(completed + "/3 TRIANGLER TEGNET");
        ...
        } else if (v.getId() == R.id.clearTriangleBtn){

            drawModel.clear();
            drawView.reset();
            drawView.invalidate();

        } else if (v.getId() == R.id.triangleContBtn){
